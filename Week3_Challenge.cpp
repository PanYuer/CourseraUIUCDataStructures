...
void PrintPair(const Pair& p) {
  std::cout << "p = (" << *p.pa << ", " <<  *p.pb << ")" << std::endl;
}
void PrintPairPointerAddresses(const Pair& p) {
  std::cout << "p addr = (" << p.pa << ", " << p.pb << ")" << std::endl;
}
...
int main() {
  ...
  Pair np(*hp);
  PrintPair(np);
  PrintPairPointerAddresses(*hp);
  PrintPairPointerAddresses(np);
...
p = (23, 42)
p = (23, 42)
p addr = (0x1231cc0, 0x1231ce0)
p addr = (0x1232d10, 0x1232d30)
